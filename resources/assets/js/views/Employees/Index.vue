  ...
  <div class="animated fadeIn">
    ...
    <div class="row">
      <div class="col-12">
          ...
          <router-link to="/employees/create" class="btn btn-primary"> <i class="fa fa-star"></i>&nbsp; Create New</router-link>
          ...
          <table class="table table-bordered table-striped table-sm">
            ...
            <tbody>
              <tr v-for="item in data" :key="item.id">
				<td>{{item.first_name+" " }}<span v-if="item.middle_name"> {{ item.middle_name +" "}}</span>{{item. last_name }}</td>
...
export default {
        ...
        methods: {        
            ...
            destroy(id) {
                axios.post('api/employees/delete/'+id ).then((response) => {                
