...
var vm = new Vue({
	...
	methods: {
		...
		fetchInterested() {
			...
			this.$http.get(this.online_url+'ads/interested/list/'+ ad_type + '/' + user_id).then((response) => {
		...
		interestedWithUser: function(event){
			...
			this.$http.get('customer/interested/' + user_id + '/' + ad_type + '/' + ad_id).then((response) => {
				...
				if(response.data[0] == '200'){
					...
					$('#'+vm.aid).css({"background-color":"green"});
					toastr.success(response.data[1], 'Added to Interested List, Thankyou', {
						timeOut: 5000
					});
		...
		removeFromInterestedList: function(event, ad){
			...
			this.$http.get('ads/interested/remove/' + user_id + '/student/' + ad.ad_id).then((response) => {
				$("#"+ad.ad_id).hide( "drop", { direction: "down" }, "slow" );
				if(response.data[0] == '200'){
					
					var index = this.ads.indexOf(ad);
					this.ads.splice(index, 1);
					
					toastr.success(response.data[1], 'Removed from Interested List, Thankyou', {
						timeOut: 5000
					});
		...
		showDetail: function(ad){
			...
			if(user_id > 0)
			window.location.href = this.online_url+'InternationalSchool/Profile/'+ad.ad_id+'/detail';
			else
			this.interested();
